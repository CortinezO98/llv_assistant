...
const CLINIC_LABEL: Record<string, string> = {
  arecibo: 'Arecibo', bayamon: 'Bayamón', latam: 'LATAM', virtual: 'Virtual'
}
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-2xl font-bold text-[#0b4c45]">Citas</h1>
          <p className="text-sm text-[#7a6a55] mt-0.5">{appointments.length} citas registradas por el bot</p>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#e5ddd4]" style={{ background: '#F5F1EB' }}>
              {['Paciente', 'Servicio', 'Fecha', 'Clínica', 'Estado', 'Acción'].map(h => (
              ...
              <tr>
                <td colSpan={6}>
                  <div className="flex flex-col items-center py-12 text-center">
                    <Icons.Stethoscope />
                    <p className="text-sm font-semibold text-[#7a6a55]">Sin citas{filter ? ` con estado "${STATUS_STYLE[filter]?.label}"` : ''}</p>
                    <p className="text-xs text-[#7a6a55]/60 mt-0.5">Las citas registradas por el bot aparecerán aquí</p>
                  </div>
                </td>
              </tr>
            ) : appointments.map(a => {
              const ss = STATUS_STYLE[a.status] || STATUS_STYLE.pending_confirm
              const StatusIcon = ss.Icon as any
              return (
                <tr key={a.id} className="table-row">
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <div className="text-sm font-semibold text-[#0b4c45]">{a.full_name}</div>
                        <div className="text-xs font-mono text-[#7a6a55]">{a.phone}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3.5">
                  ...
                  <td className="px-4 py-3.5 text-sm font-mono text-[#7a6a55]">{a.preferred_date || '—'}</td>
                  <td className="px-4 py-3.5">
                  ...
                  <td className="px-4 py-3.5">
                    {a.status === 'pending_confirm' && (
                      <button onClick={() => confirm(a.id, a.full_name)}
