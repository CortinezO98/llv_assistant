...
# Opción 1A — Cliente nuevo pérdida de peso
PREGUNTAS_NUEVO = [
...
# Opción 1B — Recompra pérdida de peso
PREGUNTAS_RECOMPRA = [
...
PREGUNTAS_QUEMADORES = [
    ...
    ("condicion",      "¿Tienes alguna condición médica o sensibilidad a estimulantes?\n_(Ej: presión alta, ansiedad, problemas cardíacos, o ninguna)_"),
    ("formato",        "¿Prefieres cápsulas o inyecciones? 💉\n1️⃣ Cápsulas\n2️⃣ Inyecciones\n3️⃣ Quiero recomendación"),
]

# Opción 3 — Péptidos
PREGUNTAS_PEPTIDOS = [
    ("objetivo",       "¿Qué resultado deseas obtener principalmente? 🎯\n1️⃣ Mejorar piel\n2️⃣ Crecimiento de cabello\n3️⃣ Antiaging\n4️⃣ Recuperación y bienestar"),
...
# Opción 6 — Limpiezas faciales
PREGUNTAS_FACIAL = [
    ("mejorar",        "¿Qué te gustaría mejorar principalmente? 🎯\n1️⃣ Acné\n2️⃣ Manchas\n3️⃣ Poros abiertos\n4️⃣ Hidratación\n5️⃣ Limpieza profunda"),
    ("tipo_piel",      "¿Cómo describirías tu tipo de piel?\n1️⃣ Grasa\n2️⃣ Mixta\n3️⃣ Seca\n4️⃣ Sensible"),
...
# Mapa de opción → (intro, preguntas, tipo_servicio)
FLUJOS_MENU = {
    "2": {
...
# Intención de entrega (opciones 1, 2, 3, 4, 7)
...
LEAD_TEMP_MAP = {
    ...
    "4": "frio", "solo": "frio", "averiguando": "frio",
}
...
_PAYMENT_INFO = {
    "puerto_rico": {"methods": ["ATH Móvil", "Tarjeta de crédito", "Apple Pay", "Zelle", "PayPal"], "primary": "ATH Móvil"},
    "latam":       {"methods": ["Zelle", "PayPal"], "primary": "Zelle"},
    "usa":         {"methods": ["Zelle", "PayPal", "Credit Card"], "primary": "Zelle"},
}
...
def _detect_lang(text: str) -> str:
    english = ["hi", "hello", "hey", "i want", "i need", "help", "what", "how", "weight", "loss"]
...
class BotService:
    ...
    def process_message(self, inbox_msg: InboxMessage) -> dict[str, Any]:
        ...
        if session.status == "completed":
            session, ctx, is_new = self._handle_completed_session(session, patient, number, text, inbox_msg)
            if session is None:
                return {"ok": True, "skipped": True, "reason": "survey_processed"}
    ...
    def _run_flow(self, text: str, ctx: dict, patient: Patient, session: Session) -> tuple[str, dict]:
        step = ctx.get("flow_step", "menu")
        msg  = text.strip()

        # Detección de escalada directa
        esc_kw  = ["agente", "asesor", "asesora", "persona", "humano", "hablar con", "agent", "quiero empezar", "queja", "reclamo"]
        ...
        if step == "menu":
            ...
            # No eligió número → mostrar menú
            return MENU_MSG if lang != "en" else MENU_MSG_EN, ctx

        # ── FILTRO PESO (opción 1) ────────────────────────────────────────────
        if step == "peso_filtro":
            ...
            elif msg in ("2", "no", "recompra", "ya he usado", "ya usé"):
        ...
        # ── PREGUNTAS GENERALES (opciones 2-9) ───────────────────────────────
        ...
        if step == "confirmacion":
            if msg.lower() in ("sí", "si", "yes", "ok", "correcto", "confirmo", "✅"):
                return self._do_escalate(ctx, patient, session, reason="Datos confirmados por el cliente", confirmed=True)
            else:
                # Corrección
                datos = ctx.get("datos", {})
                ctx["flow_step"] = "captura_datos"
                ctx["correccion"] = msg
                # Encontrar qué campo quiere corregir
                lower = msg.lower()
                ...
                return f"Dime qué dato necesitas corregir y te ayudo ✨", ctx
    ...
    def _run_preguntas(
        ...
        # Si es la primera pregunta, guardar la respuesta a la pregunta 0
        if idx == 0:
            campo = preguntas[0][0]
            respuestas[campo] = msg
            ...
            # Solo 1 pregunta → ir al siguiente paso
        else:
            ctx["respuestas"]   = respuestas
        ...
        # Para opciones que van a intención de entrega
        opcion = ctx.get("menu_opcion", "1")
        tipo_serv = ctx.get("tipo_servicio", "producto")

        if tipo == "general" and tipo_serv == "cita":
            # Servicios de cita → directo a captura de datos tipo cita
            ctx["flow_step"]   = "captura_datos"
            ctx["tipo_entrega"] = "cita"
            ctx["datos"]       = {}
            ctx["retry_count"] = {}
            ctx["campo_actual"] = None
            # Primero generar recomendación con Gemini si aplica
    ...
    def _get_gemini_recomendacion(self, respuestas: dict, ctx: dict, patient: Patient, session: Session) -> str:
        """Genera recomendación con Gemini basada en las respuestas del flujo."""
        try:
            ...
            nombres_opciones = {
                ...
                "2": "quemadores de grasa", "3": "péptidos",
                "4": "NAD+", "5": "estética",
                "6": "limpiezas faciales", "7": "sueros de vitaminas",
                "8": "rejuvenecimiento vaginal", "9": "Morpheus",
            }
    ...
    def _handle_captura_datos(
        ...
        for campo in campos_req:
            ...
            if campo == "nombre_completo" and not _validate_name(valor):
                retries = retry_count.get(campo, 0)
                if retries >= 2:
                    return self._do_escalate(ctx, patient, session, reason="Nombre inválido tras 2 intentos")
                retry_count[campo] = retries + 1
                ctx["retry_count"] = retry_count
                datos[campo] = ""
                ctx["datos"] = datos
                return "Por favor escribe tu *nombre y apellido completos* 😊\n_(Ej: María García)_", ctx

            if campo == "telefono" and not _validate_phone(valor):
                retries = retry_count.get(campo, 0)
                if retries >= 2:
                    return self._do_escalate(ctx, patient, session, reason="Teléfono inválido tras 2 intentos")
                retry_count[campo] = retries + 1
                ctx["retry_count"] = retry_count
                datos[campo] = ""
                ctx["datos"] = datos
    ...
    def _do_escalate(
        self, ctx: dict, patient: Patient, session: Session,
        reason: str = "Solicitud del cliente", confirmed: bool = False
    ) -> tuple[str, dict]:
        datos     = ctx.get("datos", {})
        respuestas = ctx.get("respuestas", {})
        ...
        nombres_opciones = {
            "1": "Pérdida de peso", "2": "Quemadores de grasa",
            "3": "Péptidos", "4": "NAD+", "5": "Estética",
            "6": "Limpiezas faciales", "7": "Sueros IV",
            "8": "Rejuvenecimiento vaginal", "9": "Morpheus",
        }
        ...
        if ctx.get("tipo_entrega"):
            tipo_labels = {"entrega": "🚚 Entrega local", "recoger": "🏥 Recoger clínica", "cita": "💉 Cita clínica"}
        ...
        logger.info("Escalada | agent=%s | session=%s | lead=%s", agent.name, session.id, lead_temp)
    ...
    def _handle_gemini(
        ...
        result      = self._gemini.process_message(
            ...
            faq_items=faq_items, patient=patient_ctx,
        )
        if result.get("function_call"):
            reply = self._handle_function_call(result["function_call"], result.get("function_args") or {}, patient, session)
        else:
    ...
    def _handle_function_call(self, fn_name: str, args: dict, patient: Patient, session: Session) -> str:
    ...
    def _fn_schedule_appointment(self, args: dict, patient: Patient, session: Session) -> str:
        appt = Appointment(
            ...
            service=args.get("service", ""), clinic=args.get("clinic", "latam"),
    ...
    def _fn_send_payment_link(self, args: dict, patient: Patient, session: Session) -> str:
        ...
        payment = Payment(patient_id=patient.id, session_id=session.id, product_service=product,amount=amount, currency="USD", payment_method=method, status="link_sent")
        self.db.add(payment); self.db.flush()
        amount_text = f"${amount:.2f} USD" if amount else "el monto indicado"
        instructions = {
            "zelle":       f"💳 *Pago por Zelle:*\nEnvía {amount_text} a: _pagos@llvclinic.com_\nEscribe tu nombre en el concepto.",
            "ath":         f"📱 *Pago por ATH Móvil:*\nEnvía {amount_text} al: _787-800-5222_\nEscribe tu nombre en el mensaje.",
            "paypal":      f"💻 *Pago por PayPal:*\nEnvía {amount_text} a: _pagos@llvclinic.com_\nSelecciona 'Amigos y familia'.",
        ...
        return f"Aquí las instrucciones de pago de *{product}*:\n\n{instructions}\n\n📸 Envía el comprobante aquí para verificarlo 😊"
    ...
    def _handle_completed_session(self, session, patient, number, text, inbox_msg):
        ...
        if ctx.get("survey_answered"):
            new_session = Session(patient_id=patient.id, whatsapp_number=number,status="active", context_json={"flow_step": "menu"})
        ...
        if score and 1 <= score <= 5:
            ...
            AnalyticsService(self.db).track(
                "satisfaction_received", session_id=session.id,
                patient_id=patient.id, agent_id=session.assigned_agent_id, score=score
            )
            ctx["survey_answered"] = True
            self._save_ctx(session, ctx)
            self._enqueue_outbox(number, f"¡Gracias por tu calificación! {'⭐' * score}\nTu opinión es muy importante para nosotros 💙✨")
    ...
    def _has_history(self, session: Session) -> bool:
        return self.db.query(MessageLog).filter(MessageLog.session_id == session.id).first() is not None
    ...
    def _get_or_create_session(self, number, patient):
        session = (self.db.query(Session).filter(Session.whatsapp_number == number, Session.status.in_(["active", "in_agent"])).order_by(Session.created_at.desc()).first())
        if not session:
            session = Session(patient_id=patient.id, whatsapp_number=number,status="active", context_json={"flow_step": "menu"})
    ...
    def _build_history(self, session):
        logs = (self.db.query(MessageLog)
                .filter(MessageLog.session_id == session.id, MessageLog.message_type == "text")
    ...
    def _log_message(self, session_id, number, direction, content, msg_type, meta_id, sent_by_bot):
        log = MessageLog(session_id=session_id, whatsapp_number=number, direction=direction,content=content, message_type=msg_type, meta_message_id=meta_id,sent_by_bot=1 if sent_by_bot else 0)
        self.db.add(log); self.db.flush()

    def _enqueue_outbox(self, to, text):
        payload = json.dumps({"to": to, "text": text})
        self.db.add(OutboxMessage(whatsapp_number=to, payload_json=payload, status="pending"))
        self.db.flush()