...
class AIOrchestrator:
    ...
    def process(self, inbox_msg: InboxMessage) -> dict[str, Any]:
        ...
        # ✅ IMPORTANTE:
        # Se valida si es conversación nueva ANTES de guardar el mensaje,
        # porque después de guardar ya existiría historial.
        is_new_conversation = not self._has_history(session)

        # ✅ Registrar mensaje entrante y conservar el ID real de BD
        inbound_log = self._log_msg(
            session.id,
            number,
            "inbound",
            text,
        ...
        # ✅ Evento realtime con message_id real para evitar pérdidas/duplicados
        realtime_manager.broadcast_sync({
            ...
            "created_at": inbound_log.created_at.isoformat()
            if inbound_log.created_at
    ...
    def _action_identify_patient(
        ...
        return OrchestratorResult(
            reply=reply,
            action_taken="identify_patient",
            success=True,
        )

    def _action_register_delivery(
        self,
        args: dict,
        patient: Patient,
        session: Session,
        ...
        if not delivery_town:
            return OrchestratorResult(
                ...
                action_taken="delivery_missing_town",
                success=False,
            )
        ...
        if not service:
            return OrchestratorResult(
                ...
                action_taken="delivery_missing_service",
                success=False,
            )
        ...
        return OrchestratorResult(
            reply=reply,
            action_taken="delivery_created",
            success=True,
        )

    def _action_register_shipment(
        self,
        args: dict,
        patient: Patient,
        session: Session,
        ...
        if not postal_address:
            return OrchestratorResult(
                ...
                action_taken="shipment_missing_address",
                success=False,
            )
        ...
        if not service:
            return OrchestratorResult(
                ...
                action_taken="shipment_missing_service",
                success=False,
            )
    ...
    def _action_evaluate_patient(
        ...
        return OrchestratorResult(
            reply=reply,
            action_taken="evaluate_patient",
            success=True,
        )

    def _action_evaluate_reorder(
        self,
        args: dict,
        patient: Patient,
        session: Session,
        ...
        if adj == "subir":
            adj_text = (
                f"Como no has tenido efectos secundarios y quieres seguir bajando, "
                f"lo ideal es *subir la dosis* 📈\n"
                f"Te recomiendo continuar con *{product}* en dosis *{new_dose}* 💉✨"
            )
        elif adj == "bajar":
            adj_text = (
                f"Gracias por contarme 😊. Lo mejor es *ajustar la dosis* "
                f"para que te sientas mejor 💉✨\n"
                f"Te recomiendo continuar con *{product}* en dosis *{new_dose}*"
            )
        elif adj == "mantenimiento":
            adj_text = (
                "¡Qué bueno leer eso! 😍 Si ya llegaste a tu peso ideal, "
        ...
        return OrchestratorResult(
            reply=reply,
            action_taken="evaluate_reorder",
            success=True,
        )

    def _action_schedule_appointment(
        self,
        args: dict,
        patient: Patient,
        session: Session,
        ...
        self.analytics.appointment_created(
            session.id,
            patient.id,
            service,
            appt.clinic,
    ...
    def _action_send_payment_link(
        ...
        payment = Payment(
            ...
            if method in ("link", "ath", "credit_card", "zelle", "paypal", "apple_pay")
            else "other",
            status="link_sent",
        )
        ...
        return OrchestratorResult(
            reply=reply,
            action_taken="payment_sent",
            success=True,
        )

    def _action_escalate_to_agent(
        self,
        args: dict,
        patient: Patient,
        session: Session,
        ...
        return OrchestratorResult(
            reply=reply,
            action_taken="agent_handoff",
            success=True,
        )

    def _action_register_payment_proof(
        self,
        args: dict,
        patient: Patient,
        session: Session,
        ...
        return OrchestratorResult(
            reply=reply,
            action_taken="payment_proof_received",
            success=True,
        )
    ...
    def _handle_payment_proof(
        ...
        result = self._action_register_payment_proof(
            {"media_id": media_id},
            patient,
            session,
        )
    ...
    def _has_history(self, session: Session) -> bool:
        ...
        return (
            ...
            .filter(MsgLog.session_id == session.id)
            .first()
            is not None
        )
